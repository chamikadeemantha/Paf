...
function AllPost() {
  ...
  const handleLike = async (postId) => {
    ...
    try {
      ...
      setFilteredPosts((prevFilteredPosts) =>
        prevFilteredPosts.map((post) => (post.id === postId ? { ...post, likes: response.data.likes } : post)),
      )
    } catch (error) {
  ...
  const handleAddComment = async (postId) => {
    ...
    try {
      ...
      setPosts((prevPosts) =>
        prevPosts.map((post) => (post.id === postId ? { ...post, comments: response.data.comments } : post)),
      )

      setFilteredPosts((prevFilteredPosts) =>
        prevFilteredPosts.map((post) => (post.id === postId ? { ...post, comments: response.data.comments } : post)),
      )
          ...
          <div className={`post_card_continer ${viewMode}`}>
            {filteredPosts.length === 0 ? (
              <div className="not_found_box">
                ...
                <button className="not_found_btn" onClick={() => (window.location.href = "/addNewPost")}>
                  Create New Post
                </button>
              </div>
            ) : (
                ...
                <div key={post.id} className="post_card">
                  ...
                  <div className="user_details_card_di">
                    ...
                    <div className="description-container">
                      <p className="card_post_description" style={{ whiteSpace: "pre-line" }}>
                        {expandedDescriptions[post.id] ? post.description : truncateText(post.description)}
                      </p>
                      {post.description && post.description.length > 150 && (
                        <button className="description-toggle-btn" onClick={() => toggleDescriptionExpand(post.id)}>
                    ...
                    <button className="recipe-details-toggle" onClick={() => togglePostDetails(post.id)}>
                      {expandedPosts[post.id] ? "Hide Recipe Details" : "Show Recipe Details"}
                    </button>
                      ...
                      <div className="recipe-details">
                        ...
                        {post.recipe && (
                          <div className="recipe-steps-section">
                            <h4>Recipe Steps</h4>
                            <p style={{ whiteSpace: "pre-line" }}>{post.recipe}</p>
                          </div>
                        )}
                      </div>
                    )}
                      ...
                      <div key={comment.id} className="coment_full_card">
                        ...
                        <div className="coment_action_btn">
                            ...
                            <>
                              {editingComment.id === comment.id ? (
                                <>
                                  ...
                                  />
                                  <TbPencilCancel className="coment_btn" onClick={() => setEditingComment({})} />
                                </>
                              ) : (
                                <>
                                  <GrUpdate
                          ...
                          {post.userID === loggedInUserID && comment.userID !== loggedInUserID && (
                            <button className="coment_btn" onClick={() => handleDeleteComment(post.id, comment.id)}>
                              Delete
                            </button>
                          )}
                        </div>
